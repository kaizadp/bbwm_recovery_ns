...
gg_n = 
  ...
  annotate("text", label = "pre-treatment", x = as.Date("1988-01-01"), y = 1.8, angle = 90,size=4, hjust = "right")+
  annotate("text", label = "recovery", x = as.Date("2017-09-15"), y = 1.8, angle = 90,size=4, hjust = "right")+ 
  theme_kp()+
  NULL
...
gg_s = 
  ...
  mutate(date2 = as.Date(paste(Year,Month,Day, sep = "-"))) %>% 
  ggplot(aes(x = date2, color = Watershed,shape=Watershed, size=Watershed))+
  geom_point(aes(y = SO4_S), stroke=0.5)+
  #ylab("NO3-N conc, mg/L")+
  ylab(expression(bold("SO"[4]*"-S, mg L"^{-1})))+
  xlab("")+
  scale_shape_manual(values = c(1,19))+
  ...
  geom_vline(xintercept = as.Date("1989-06-30"),linetype="dashed")+
  geom_vline(xintercept = as.Date("2016-10-30"),linetype="dashed")+
  annotate("text", label = "pre-treatment", x = as.Date("1988-01-01"), y = 4, angle = 90,size=4, hjust = "right")+
  annotate("text", label = "recovery", x = as.Date("2017-09-15"), y = 4, angle = 90,size=4, hjust = "right")+ 
  theme_kp()+
  NULL
...
```{r}
combined_flux %>% 
  ggplot(aes(x = S_in, y = S_out, color = WY))+
  geom_path()+geom_point()+
  facet_grid(.~Watershed, scales = "free")

combined_flux %>% 
  ggplot(aes(x = N_in, y = N_out, color = WY))+
  geom_path()+geom_point()+
  facet_grid(.~Watershed, scales = "free")

combined_flux %>% 
  ggplot(aes(x = S_in+N_in, y = S_out+N_out, color = WY))+
  geom_path()+geom_point()+
  facet_grid(.~Watershed, scales = "free")
