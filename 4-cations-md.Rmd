...
```{r camgns3, fig.width=7, fig.height=6}
wy_label2 = tribble(
~Watershed, ~label, ~x, ~y,
#"WB", 1989, 128, 110,
#"WB", 1990, 145, 118,
...
ggplot(data = camg_ns %>% filter(Watershed=="WB"),
  ...
  geom_path(data = camg_ns %>% filter(Watershed=="EB"),
            color = "black")+  
  geom_point(data = camg_ns %>% filter(Watershed=="EB"),
            size=2,stroke=1.5, shape = 19, color="black")+
  ...
  labs (y = expression(bold("(Ca"^+2 * "+ Mg"^+2* ") ("*mu*"eq L"^-1*")" )),
        x = expression(bold("(NO"[3]^-{}*"+SO"[4] ^-2* ") ("*mu*"eq L"^-1*")" )),
        shape = "", color = "", fill = "West Bear")+
  
  # add arrows ----
  annotate("segment", x = 100, y = 125, xend = 37, yend = 80, size=1, color = "grey60",
           arrow = arrow(length = unit(0.3, "cm")))+
  annotate("segment", x = 120, y = 145, xend = 220, yend = 190, size=1, color = "grey60",
           arrow = arrow(length = unit(0.3, "cm")))+
  annotate("segment", x = 260, y = 100, xend = 180, yend = 60, size=1, color = "grey60",
           arrow = arrow(length = unit(0.3, "cm")))+
  annotate("curve", x = 260, y = 190, xend = 280, yend = 120, curvature = -0.4, size=1, color = "grey60",
           arrow = arrow(length = unit(0.3, "cm")))+
